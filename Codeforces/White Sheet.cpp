...
int main(){
	...
	pair<pll, pll> in2 = inter(arr[0], arr[1], arr[4], arr[5]);
	pair<pll, pll> intersect = inter(in1.fi, in1.se, in2.fi, in2.se);
	
	ll white = area(arr[0], arr[1]);
	ll ar1 = area(in1.fi, in1.se);
	ll ar2 = area(in2.fi, in2.se); 
	ll minus = area(intersect.fi, intersect.se);
	//cout << white << " " << ar1 << " " << ar2 << " " << minus << ed;
	if(white > ar1+ar2-minus) cout << "YES";
	else cout << "NO";
}
