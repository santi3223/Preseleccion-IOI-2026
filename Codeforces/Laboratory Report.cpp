//5 de Abril 2026 21:34
//Decimals to Integers version
#include <bits/stdc++.h>
using namespace std;
#define ll long long
#define vl vector<ll>
...
unordered_map<string, vector<pair<ll, pair<vl, string>>>> flag;
/*  test                          1         2     3
...
ll convertit(string cur){
	...
	if(dott == -1) au += string(6, '0');
	else{
		ll quant = cur.size()-1-dott;
		au += string(6-quant, '0');
	}
	return stoll(au);
}

pair<ll, vl> type(string S){
	string s = "";
	ff(i, 0, S.size()){
		if(S[i] == ' ') continue;
		s += S[i];
